...
public class imagecoverp5tint extends PApplet {
...
int COVERWIDTH = 200;
int COVERHEIGHT = 300;
float COVERRATIO = PApplet.parseFloat(COVERWIDTH) / PApplet.parseFloat(COVERHEIGHT);
int MARGIN = 5;
int IMAGEMARGIN = 10;
int TITLEHEIGHT = 50;
int AUTHORHEIGHT = 25;
...
public void setup() {
  ...
  if (!is_command_line) {
    ...
    int fontSize = PApplet.parseInt(title_suffix);
    ...
    fontSize = PApplet.parseInt(author_suffix);
...
public void executeDraw() {
  ...
  if (baseFolder!="") {
    int tempID = 0;
    if (!is_command_line) tempID = PApplet.parseInt(cp5.get(Textfield.class,"inputID").getText());
  ...
  if (batch) {
    ...
    if (!is_command_line && currentBook >= bookList.length) {
      ...
      refresh = false;
      int endTime = millis();
      int seconds = round((endTime - startTime)*.001f);
...
public PImage createCover(int index) {
  pg.clear();
  pg.beginDraw();
...
public void getBookImages(JSONArray json_images) {
  images.clear();
  ...
  for (i=0;i<l;i++) {
    ...
    if (ratio >= COVERRATIO) {
      ...
      imgBW = temp.get(PApplet.parseInt((temp.width-COVERWIDTH)*.5f)+MARGIN, MARGIN, COVERWIDTH, COVERHEIGHT);
      // println("wider");
    } else {
      // temp.resize(0, maxHeight);
      temp.resize(COVERWIDTH + MARGIN*3, 0);
      imgBW = temp.get(MARGIN, PApplet.parseInt((temp.height-COVERHEIGHT)*.5f)+MARGIN, COVERWIDTH, COVERHEIGHT);
      // println("taller");
    }
...
public JSONObject getBook(String[] list, int index) {
  int l = list.length;
  int i;
...
public int getIndexForBookId(int id) {
...
public void processColors() {
  ...
  int colorSeed = PApplet.parseInt(map(counts, 6, MAXTITLECHAR+MAXAUTHORCHAR, 30, 260));
  colorMode(HSB, 360, 100, 100);
  int lightColor = color((colorSeed+180)%360, baseSaturation-20, baseBrightness+40);
  int darkColor = color(colorSeed, baseSaturation, baseBrightness-20);
...
public void drawArtwork(PGraphics g, int index, int _x, int _y) {
...
public void drawBackground(PGraphics g, int x, int y) {
  g.fill(BACKGROUNDCOLOR);
  g.rect(x, y, COVERWIDTH, COVERHEIGHT);
}

public void drawText(PGraphics g, int x, int y) {
  //\u2026
  g.noStroke();
  // for text box
  g.fill(textColor);
...
public void controlEvent(ControlEvent theEvent) {
  ...
  if(theEvent.isGroup() && theEvent.name().equals("titleList")){
    ...
    int fontSize = PApplet.parseInt(suffix);
    println("size:" + suffix);
...
public void folderSelected(File selection) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "imagecoverp5tint" };
    if (passedArgs != null) {
