...
class CustomListUploadScript(StaticFeedScript):
    ...
    def fetch_editable_list(self, list_name, list_id, save_option):
        ...
        """
        custom_list = CustomList.find(self._db, list_id)
        ...
        if custom_list.data_source != self.source:
    ...
    def works_from_source(self, source_csv):
        ...
        self._update_works(selections)

        return works_qu, youth_works_qu

    def _update_works(self, selections_dict):
        ...
        featured_work_identifiers = selections_dict['featured']
        for identifier in featured_work_identifiers:
            work = identifier.licensed_through.work
            featured_quality = Configuration.minimum_featured_quality()
            if not work.quality > featured_quality:
                # This creates a single measurement attributed to
                # this DataSource and normalizes its value to equal the
                # minimum requirements for being featured.
                measurement = identifier.add_measurement(
                    self.source, Measurement.RATING, 1
                )
                measurement._normalized_value = featured_quality
                # Then recalculates the work quality to use _only_ this
                # measurement.
                work.calculate_quality([identifier.id])
        self._db.flush()

    def edit_list(self, custom_list, works_qu, save_option):
        ...
        if save_option in self.ADD_OPTIONS:
            ...
            [custom_list.add_entry(e) for e in input_editions]
            return
